...
const handleAuthentication = (expiresIn : number , email : string , id : string , token :string) => {
 ...
 return new AuthActions.Login({email : email , userId : id , token : token , expirationDate : expirationDate , redirect : true});
...
export class AuthEffects{
    ...
    authSuccess = this.actions$.pipe(ofType(AuthActions.LOGIN) , tap(
        (authSuccessActions : AuthActions.Login) =>  {
    ...
    autoLogin = this.actions$.pipe(ofType(AuthActions.AUTO_LOGIN) , map(() =>  {
        const userData : {
           ...
           if(loadUser.token){
            ...
            return new AuthActions.Login({email : userData.email , userId : userData.id , token : userData._token , 
